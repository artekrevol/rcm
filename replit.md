...
- **Provider Guide Ingestion** (formerly "Payer Manual Ingestion"): Admin pipeline to ingest payer source documents via URL or PDF upload, extracting billing rules using AI. Phase 1 multi-document schema: `payer_manuals` table now carries `document_type` (admin_guide / supplement / pa_list / reimbursement_policy / medical_policy / bulletin / contract / fee_schedule), `parent_document_id` (nullable self-ref FK for supplements), `effective_start` (DATE), `effective_end` (DATE nullable = currently in effect). GET list returns `parent_document_name` (JOIN) and `supplement_count` (subquery). UI renamed to "Provider Guide Ingestion" with document-type color badges, effective-date display, supplement indentation, and an expanded Add Source Document dialog. **Prompt B1 vocabulary expansion**: `rule_kinds` reference table seeded with 15 codes organized in 4 UI groups (Operational, Technical, Timeframe, Strategic) — 14 active in extraction, `risk_adjustment_hcc` seeded with `active_in_extraction=FALSE` for Phase 3. Extraction pipeline expanded from 4 to 14 section kinds. `modifiers` rows migrated to `modifiers_and_liability` schema with `needs_reverification=TRUE` flag. Claude extraction prompts upgraded: 9 new result interfaces, upgraded `ModifiersAndLiabilityResult` with `conditions_required`, `conditions_excluded`, `liability_assignment`, `appeal_path_if_denied`. Coverage matrix API updated to emit 14 section-kind percentages. Rules engine evaluation query updated to use `modifiers_and_liability` section type. PDF vision prompt updated to mention all 14 active section kinds. Keyword arrays: `submission_timeframe` uses directional bigrams only; `decision_timeframe`/`documentation_timeframe` use directional bigrams; `notification_event` owns all "X notification" phrases; `member_notice` scoped to ABN/NOMNC/IDN/written consent/termination only; `referrals` adds specialty-care phrases; `place_of_service` adds POS-22/POS-23/outpatient hospital.
