...
const FILES = [
  ...
  path.join(__dirname, '../attached_assets/ccioph-v320r0-f2_1777592939297.zip'),
  path.join(__dirname, '../attached_assets/ccioph-v320r0-f3_1777592939297.zip'),
  path.join(__dirname, '../attached_assets/ccioph-v320r0-f4_1777592939296.zip'),
];
...
async function ingestFile(filePath) {
  ...
  // Find first data row (starts with digit)
  let dataStart = 0;
  ...
  // Parse + deduplicate: keep last occurrence of (col1, col2, effDate)
  const deduped = new Map(); // key → row array
  let parsedTotal = 0;
  ...
  for (let i = dataStart; i < rawLines.length; i++) {
    ...
    // Tab layout: col1, col2, *, effDate, delDate, modifier, rationale
...
async function main() {
  ...
  for (let i = 0; i < FILES.length; i++) {
    ...
    const destDir = path.join(os.tmpdir(), `cci_q2b_f${i+1}`);
