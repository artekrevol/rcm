...
export async function registerRoutes(server: Server, app: Express): Promise<void> {
  ...
  try {
    ...
    // Always enforce situational for TWVACCN — no condition on current value so
    // this self-heals even if the column was added as NULL or 'required' in prod.
    await pool.query(`UPDATE payers SET referring_provider_policy = 'situational' WHERE payer_id = 'TWVACCN'`);
    // Optum VA CCN only — Optum commercial plans still require the referring provider.
    await pool.query(`UPDATE payers SET referring_provider_policy = 'situational' WHERE LOWER(name) LIKE '%optum%' AND payer_classification = 'va_community_care'`);
    ...
    if (psdSeedCnt === 0) {
      ...
      await pool.query(`
        ...
        ('mei-tw-mod-001', 'manual-triwest-001', 'modifiers_and_liability',
        ...
        ('mei-mc-mod-001', 'manual-medicare-001', 'modifiers_and_liability',
        ...
        ('mei-ae-mod-001', 'manual-aetna-001', 'modifiers_and_liability',
    ...
    await seederLog('column', 'billing_periods', 'period_charge');
    await pool.query(`ALTER TABLE billing_periods ADD COLUMN IF NOT EXISTS period_charge NUMERIC(12,2) DEFAULT 0`).catch(() => {});
    await seederLog('column', 'claims', 'attending_provider_id');
    await pool.query(`ALTER TABLE claims ADD COLUMN IF NOT EXISTS attending_provider_id UUID`).catch(() => {});
  ...
  app.post("/api/billing/claims/:id/submit-stedi", requireRole("admin", "rcm_manager"), async (req, res) => {
    try {
      ...
      // ── 837I institutional claim dispatch ───────────────────────────────────
      // Claims generated by /api/hh/billing-periods/:id/generate-claim carry
      // claim_transaction_set='837I'.  Re-generate EDI from stored billing period
      // data and submit via the same Stedi transport used for 837P.
      if (c.claim_transaction_set === '837I') {
        if (!c.billing_period_id) {
          return res.status(422).json({ success: false, error: "837I claim has no linked billing period." });
        }

        const bpResult = await db.query(
          `SELECT bp.*, e.start_of_care_date, e.primary_diagnosis, e.cert_period_start
           FROM billing_periods bp
           JOIN episodes e ON e.id = bp.episode_id
           WHERE bp.id=$1 AND bp.organization_id=$2`,
          [c.billing_period_id, c.organization_id],
        );
        const bp837i = bpResult.rows[0];
        if (!bp837i) {
          return res.status(422).json({ success: false, error: "Billing period not found for this claim." });
        }

        // Load attending provider (stored on claim when generated)
        let attending837i: any = { first_name: prov.first_name, last_name: prov.last_name, npi: prov.npi || ps.primary_npi || "" };
        if (c.attending_provider_id) {
          const pr = await db.query(
            `SELECT first_name, last_name, npi FROM providers WHERE id=$1`,
            [c.attending_provider_id],
          );
        ...
        const visitRowsResult = await db.query(
        ...
        const revCodeMap: Record<string, string> = {
        ...
        const visitLines837i = visitRowsResult.rows.map((r: any) => ({
          revenueCode: revCodeMap[r.discipline] || '0559',
        ...
        const { edi: edi837i, rpTransmitted: rp837i } = selectAndGenerate837I({
          ...
          totalCharge: parseFloat(bp837i.period_charge || "0"),
          hippsCode: bp837i.hipps_code || "",
          visitLines: visitLines837i,
          oasisDate: bp837i.oasis_date || "",
          fipsCounty: bp837i.fips_county || "",
          cbsaCode: bp837i.cbsa_code || null,
          utnNumber,
          patientStatusCode: "30",
          admission: {
            socDate: bp837i.start_of_care_date,
            firstVisitDate: bp837i.period_start,
            principalDiagnosis: bp837i.primary_diagnosis || "",
          },
          patient: {
            first_name: pat.first_name || "",
            last_name: pat.last_name || "",
            ...
            member_id: pat.member_id || pat.insurance_id || "",
            sex: pat.sex || null,
            address: (typeof pat.address === 'object' && pat.address) ? (pat.address as any).street || null : null,
            city: (typeof pat.address === 'object' && pat.address) ? (pat.address as any).city || pat.city || null : pat.city || null,
            state: (typeof pat.address === 'object' && pat.address) ? (pat.address as any).state || pat.state || null : pat.state || null,
            zip: (typeof pat.address === 'object' && pat.address) ? (pat.address as any).zip || pat.zip || null : pat.zip || null,
          },
          practice: {
            name: ps.practice_name || "",
            legal_name: ps.legal_name || null,
            npi: ps.agency_npi || ps.primary_npi || "",
            tax_id: ps.agency_tax_id || ps.tax_id || "",
            taxonomy_code: ps.taxonomy_code || "251E00000X",
            address: practiceAddr837i.line1 || practiceAddr837i.address || practiceAddr837i.street || "",
            city: practiceAddr837i.city || "",
            state: practiceAddr837i.state || "",
            zip: practiceAddr837i.zip || "",
            phone: ps.phone || "",
          },
          attendingProvider: {
            first_name: attending837i.first_name || "",
            last_name: attending837i.last_name || "",
            npi: attending837i.npi || "",
          },
          payer: {
            name: payerInfo.name,
            payer_id: (payerInfo as any).payer_id || "UNKNOWN",
            stedi_payer_id: payerInfo.stedi_payer_id,
            claim_filing_indicator: (payerInfo as any).claim_filing_indicator || null,
            member_id_qualifier: (payerInfo as any).member_id_qualifier || null,
          },
        });

        await db.query(
          `UPDATE claims SET referring_provider_transmitted=$1 WHERE id=$2`,
          [JSON.stringify(rp837i), c.id],
        ).catch(() => {});

        const result837i = await stediSubmitClaim({
          ediContent: edi837i,
          claimId: c.id,
          hasUserSession: !!(req.user),
          userAgent: req.headers["user-agent"],
        ...
        if (result837i.success) {
          await db.query(
            `UPDATE claims SET status='submitted', updated_at=NOW() WHERE id=$1`,
            [c.id],
          ).catch(() => {});
          await db.query(
            `UPDATE billing_periods SET period_status='submitted', updated_at=NOW() WHERE id=$1 AND organization_id=$2`,
            [c.billing_period_id, c.organization_id],
          ).catch(() => {});
        }

        return res.json({
          success: result837i.success,
          claimId: c.id,
          transactionId: result837i.transactionId,
          controlNumber: result837i.controlNumber,
          status: result837i.status,
          validationErrors: result837i.validationErrors,
          error: result837i.error,
          blockedBy: result837i.blockedBy,
        });
      }
      // ── END 837I dispatch ────────────────────────────────────────────────────
    ...
    try {
      ...
      const context = await withTenantTx(async (client) => {
        ...
        const { rows: visitRows } = await client.query(
          `SELECT discipline, COUNT(*) AS visit_count
           FROM episode_visits
           WHERE billing_period_id=$1 AND organization_id=$2
           GROUP BY discipline`,
          [periodId, orgId],
        ...
        return { bp, patient, payer, provider, ps, pcr: pcr || null, visitRows };
      }, orgId);

      if (!context) return res.status(404).json({ error: "Billing period not found" });
      const { bp, patient, payer, provider, ps, pcr, visitRows } = context;
      ...
      // Map episode visit discipline counts to 837I revenue lines
      const disciplineToRevCode: Record<string, string> = {
        'SN': '0551', 'skilled_nursing': '0551', 'skilled-nursing': '0551',
        'PT': '0421', 'physical_therapy': '0421', 'physical-therapy': '0421',
        'OT': '0431', 'occupational_therapy': '0431', 'occupational-therapy': '0431',
        'ST': '0441', 'speech_therapy': '0441', 'speech-therapy': '0441',
      ...
      const visitLines = (visitRows || []).map((row: any) => ({
        revenueCode: disciplineToRevCode[row.discipline] || '0559',
        visitCount: Number(row.visit_count),
        charge: 0,
      }));
      const totalCharge = parseFloat(bp.period_charge || "0");
      ...
      const claimId = await withTenantTx(async (client) => {
        const { rows: [claim] } = await client.query(
          `INSERT INTO claims (
             id, organization_id, patient_id, status, payer_id, payer_name,
             service_date, place_of_service, icd10_codes, service_lines, amount,
             claim_transaction_set, claim_frequency_code, billing_period_id,
             attending_provider_id, created_at, updated_at
           ) VALUES (
             gen_random_uuid(), $1, $2, 'draft', $3, $4, $5, '12', $6, '[]'::jsonb, $7, '837I', $8, $9, $10, NOW(), NOW()
           ) RETURNING id`,
          [
  ...
  app.post("/api/hh/noa/:id/submit", requireHH("home_health_skilled"), async (req, res) => {
    try {
      ...
      if (isStediConfigured() && stediSubmit) {
        stediResponse = await stediSubmit({
          ediContent: noaResult.edi,
          claimId: noaId,
          hasUserSession: !!(req.user),
          userAgent: req.headers['user-agent'],
        });
        ...
        const { rows: [row] } = await client.query(
          `UPDATE billing_periods
           SET hipps_code   = COALESCE($1, hipps_code),
               ...
               period_charge = COALESCE($5::numeric, period_charge),
               updated_at   = NOW()
           WHERE id=$6 AND organization_id=$7 RETURNING *`,
          [hipps_code || null, oasis_date || null, cbsa_code || null, fips_county || null,
           period_charge != null ? String(period_charge) : null,
           req.params.id, orgId],
        );
        return row;
      }, orgId);
